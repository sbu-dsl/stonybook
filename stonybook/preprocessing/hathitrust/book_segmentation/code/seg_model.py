...
class Model:
	...
	def network(self, x, weightss, biases, dropout, seq_length):
		...
		lstm_fw_cell = tf.nn.rnn_cell.BasicLSTMCell(self.n_hidden, forget_bias=1.0, state_is_tuple=True)
		lstm_fw_multicell = tf.nn.rnn_cell.MultiRNNCell([lstm_fw_cell] * self.number_of_layers, state_is_tuple=True)

		lstm_bw_cell = tf.nn.rnn_cell.BasicLSTMCell(self.n_hidden, forget_bias=1.0, state_is_tuple=True)
		lstm_bw_multicell = tf.nn.rnn_cell.MultiRNNCell([lstm_bw_cell] * self.number_of_layers, state_is_tuple=True)
