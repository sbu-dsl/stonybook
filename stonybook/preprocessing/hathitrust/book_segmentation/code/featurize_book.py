...
class book:
    ...
    def write(self, outfile):
        ...
        for page, pagefeatures in sorted(self.pages.items()):
            out.write("%s\t%s\t%s\n" % (self.idd, page, " ".join(["=".join([str(key), str(val)]) for key, val in list(pagefeatures.items())])))
    ...
    def getbook(self, zippath):
        ...
        # return zf.namelist()
        return sorted(os.listdir(zippath))
    ...
    def createPageFeatures(self):
        ...
        for i in range(len(pages)):
            text = self.getpagewithnum(self.zippath, pages[i])
